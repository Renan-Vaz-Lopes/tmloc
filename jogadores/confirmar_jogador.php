...
if (isset($_GET['token'])) {
    ...
    if ($resultado && $resultado->num_rows > 0) {
        ...
        if ($dados['segundos_passados'] <= 300) {
            ...
            $sql_insert = "INSERT INTO jogadores (nome, email, senha, estado, cidade, descricao_contato, nivel)
                           VALUES ('{$dados['nome']}', '{$dados['email']}', '{$dados['senha']}', '{$dados['estado']}', '{$dados['cidade']}', '{$dados['descricao_contato']}', '{$dados['nivel']}')";
            if ($mysqli->query($sql_insert)) {
                // Remove o registro da tabela temporária
                $mysqli->query("DELETE FROM jogadores_temp WHERE token = '$token'");
                $mensagem_sucesso = '
    <div>
